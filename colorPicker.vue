...
<template lang="html">
  <div class="m-colorPicker" ref="colorPicker" v-on:click="event => { event.stopPropagation() }">
    ...
    <!--<div class="colorBtn"
      ...
      v-bind:class="{ disabled: openStatus }"
    ></div>-->
     <div class="colorBtn" ref="colorBtn"
      v-bind:style="`background-color: ${showColor}`"
    ...
    <!-- 颜色色盘 -->
    <div class="box" v-bind:class="{ open: openStatus }" ref="colorBox">
      <div class="hd">
...
.m-colorPicker {
  ...
  .box {
    z-index: 9999; // right: -49px;
    display: flex;
    flex-direction: column;
    position: absolute;
    width: 190px;
    background: #fff;
    border: 1px solid #ddd; // visibility: hidden;
    border-radius: 2px;
    margin-top: 2px;
    padding: 10px;
    padding-bottom: 5px;
